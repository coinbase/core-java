...
public abstract class CoinbaseNetHttpClient implements CoinbaseClient {
  ...
  public <T> T sendRequest(
    ...
    if (httpMethod == HttpMethod.GET || httpMethod == HttpMethod.DELETE) {
      ...
      authHeaders =
          credentials.generateAuthHeaders(httpMethod.name(), URI.create(callUrl), jsonPayload);
      requestBuilder.header("Content-Type", "application/json");
    }

    URI uri = URI.create(callUrl);

    requestBuilder = requestBuilder.uri(uri).method(httpMethod.name(), bodyPublisher);
                  ...
                  valueNode.forEach(
                      arrayElement -> {
                        ...
                        if (!arrayElement.isNull()) {
                          String value =
                              URLEncoder.encode(arrayElement.asText(), StandardCharsets.UTF_8);
